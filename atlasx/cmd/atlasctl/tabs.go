...
type commandTabsClient interface {
	...
	SetTitle(string, string) (tabs.TitleUpdateResult, error)
	...
	OpenDevToolsPanelWindowToWindows(int, string) (tabs.DevToolsPanelWindowToWindowsResult, error)
...
func runTabs(args []string) error {
	if len(args) == 0 {
		return errors.New("missing tabs subcommand: list, search, windows, open, open-window, open-in-window, move-to-window, move-to-new-window, merge-window, open-devtools, open-devtools-in-window, open-devtools-panel, open-devtools-panel-in-window, open-devtools-window-to-windows, open-devtools-panel-window-to-windows, open-devtools-panel-window-into-window, open-devtools-window-into-window, close-duplicates, activate-window, close-window, set-window-state, set-window-bounds, set-title, activate, close, navigate, capture, auth-mode, extract-context, selection, devtools, devtools-panel, emulate-device, suggest, agent-plan, agent-execute, memories, groups, organize, organize-window, organize-group-to-window, organize-group-into-window, organize-to-windows, organize-into-window, organize-window-to-windows, organize-window-into-window, organize-window-group-to-window, organize-window-group-into-window, recommend-context")
	}
	...
	case "open-devtools":
		...
		fmt.Printf("id=%s type=%s title=%q url=%s\n", target.ID, target.Type, target.Title, target.URL)
		return nil
	case "open-devtools-in-window":
		if len(args) < 3 {
			return errors.New("missing target id or window id for tabs open-devtools-in-window")
		}
		windowID, err := strconv.Atoi(args[2])
		if err != nil {
			return fmt.Errorf("invalid window id %q", args[2])
		}
		result, err := client.OpenDevToolsInWindow(args[1], windowID)
		if err != nil {
			return err
		}
		fmt.Printf(
	...
	case "open-devtools-window-into-window":
		...
		return nil
	case "open-devtools-window-to-windows":
		if len(args) < 2 {
			return errors.New("missing source window id for tabs open-devtools-window-to-windows")
		}
		sourceWindowID, err := strconv.Atoi(args[1])
		if err != nil {
			return fmt.Errorf("invalid source window id %q", args[1])
		}
		result, err := client.OpenDevToolsWindowToWindows(sourceWindowID)
		if err != nil {
			return err
		}
		...
		for index, opened := range result.OpenedTargets {
			fmt.Printf(
				"index=%d source_target_id=%s id=%s type=%s title=%q url=%s\n",
				index,
				opened.SourceTargetID,
				opened.Target.ID,
				opened.Target.Type,
		...
		return nil
	case "open-devtools-panel-window-to-windows":
		if len(args) < 3 {
			return errors.New("missing source window id or panel for tabs open-devtools-panel-window-to-windows")
		}
		sourceWindowID, err := strconv.Atoi(args[1])
		if err != nil {
			return fmt.Errorf("invalid source window id %q", args[1])
		}
		result, err := client.OpenDevToolsPanelWindowToWindows(sourceWindowID, args[2])
		if err != nil {
			return err
		}
		fmt.Printf("source_window_id=%d panel=%s returned=%d\n", result.SourceWindowID, result.Panel, result.Returned)
		for index, opened := range result.OpenedTargets {
			fmt.Printf(
				"index=%d source_target_id=%s id=%s type=%s title=%q url=%s\n",
				index,
		...
		return nil
	case "open-devtools-panel-window-into-window":
		if len(args) < 4 {
			return errors.New("missing source window id or panel or target window id for tabs open-devtools-panel-window-into-window")
		}
		sourceWindowID, err := strconv.Atoi(args[1])
		if err != nil {
			return fmt.Errorf("invalid source window id %q", args[1])
		}
		targetWindowID, err := strconv.Atoi(args[3])
		if err != nil {
			return fmt.Errorf("invalid target window id %q", args[3])
		}
		result, err := client.OpenDevToolsPanelWindowIntoWindow(sourceWindowID, args[2], targetWindowID)
		if err != nil {
			return err
		}
		fmt.Printf("source_window_id=%d panel=%s target_window_id=%d returned=%d\n", result.SourceWindowID, result.Panel, result.TargetWindowID, result.Returned)
		for index, opened := range result.OpenedTargets {
			fmt.Printf(
				"index=%d source_target_id=%s activated_target_id=%s id=%s type=%s title=%q url=%s\n",
				index,
	...
	case "set-title":
		if len(args) < 3 {
			return errors.New("missing target id or title for tabs set-title")
		}
		result, err := client.SetTitle(args[1], args[2])
		if err != nil {
			return err
		}
	...
	case "auth-mode":
		return runTabsAuthMode(client, args[1:])
	...
	case "agent-execute":
		return runTabsAgentExecute(paths, client, args[1:])
	case "memories":
		return runTabsMemories(paths, client, args[1:])
	case "groups":
		return runTabsGroups(client)
	case "organize":
