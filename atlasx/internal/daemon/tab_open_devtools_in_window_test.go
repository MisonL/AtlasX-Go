...
func TestTabsOpenDevToolsInWindowEndpoint(t *testing.T) {
	...
	for _, fragment := range []string{`"window_id":7`, `"activated_target_id":"tab-7"`, `"id":"devtools-window-tab"`} {
...
func TestTabsOpenDevToolsInWindowEndpointRejectsInvalidWindowID(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	...
	request := httptest.NewRequest(
		http.MethodPost,
		"/v1/tabs/open-devtools-in-window",
		bytes.NewBufferString(`{"id":"tab-1","window_id":0}`),
	)
	recorder := httptest.NewRecorder()
