...
func serveTabOpenDevToolsInWindow(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		writeError(w, http.StatusMethodNotAllowed, fmt.Errorf("method %s is not allowed", r.Method))
		return
	}

	var request tabOpenDevToolsInWindowRequest
	...
	result, err := client.OpenDevToolsInWindow(request.ID, request.WindowID)
	if err != nil {
