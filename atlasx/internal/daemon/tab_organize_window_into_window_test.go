...
func TestTabOrganizeWindowIntoWindowReturnsGroups(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/v1/tabs/organize-window-into-window", bytes.NewBufferString(`{"source_window_id":11,"target_window_id":21}`))
	...
	for _, fragment := range []string{`"source_window_id":11`, `"target_window_id":21`, `"returned":2`, `"group_id":"host:chatgpt.com"`} {
...
func TestTabOrganizeWindowIntoWindowRejectsInvalidTargetWindowID(t *testing.T) {
	t.Setenv("HOME", t.TempDir())

	restoreDaemonHooks(t, &stubTabsClient{})

	request := httptest.NewRequest(http.MethodPost, "/v1/tabs/organize-window-into-window", bytes.NewBufferString(`{"source_window_id":11,"target_window_id":0}`))
	recorder := httptest.NewRecorder()
