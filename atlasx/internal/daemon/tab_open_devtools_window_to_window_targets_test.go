...
func TestTabsOpenDevToolsWindowToWindowsEndpoint(t *testing.T) {
	...
	for _, fragment := range []string{`"source_window_id":11`, `"returned":1`, `"opened_targets":[`} {
...
func TestTabsOpenDevToolsWindowToWindowsEndpointRejectsInvalidSourceWindowID(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	...
	request := httptest.NewRequest(
		http.MethodPost,
		"/v1/tabs/open-devtools-window-to-windows",
		bytes.NewBufferString(`{"source_window_id":0}`),
	)
	recorder := httptest.NewRecorder()
