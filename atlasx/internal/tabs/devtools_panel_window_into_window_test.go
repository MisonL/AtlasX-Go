...
func TestOpenDevToolsPanelWindowIntoWindowOpensPanelInTargetWindow(t *testing.T) {
	...
	result, err := client.OpenDevToolsPanelWindowIntoWindow(9, "network", 7)
	if err != nil {
		t.Fatalf("open devtools panel window into window failed: %v", err)
	}
	if result.SourceWindowID != 9 || result.Panel != "network" || result.TargetWindowID != 7 || result.Returned != 2 {
	...
	for _, opened := range openURLs {
		if !strings.Contains(opened, "panel=network") {
			t.Fatalf("expected panel in url: %s", opened)
		}
	}
}

func TestOpenDevToolsPanelWindowIntoWindowRejectsBlankPanel(t *testing.T) {
	upgrader := websocket.Upgrader{}
	mux := http.NewServeMux()
	server := httptest.NewServer(mux)
	defer server.Close()
	...
	client := Client{baseURL: server.URL, httpClient: *server.Client()}
	if _, err := client.OpenDevToolsPanelWindowIntoWindow(9, " ", 7); err == nil {
		t.Fatal("expected open devtools panel window into window to fail")
	} else if !strings.Contains(err.Error(), "panel is required") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestOpenDevToolsPanelWindowIntoWindowRejectsSameWindowID(t *testing.T) {
	client := Client{}
	if _, err := client.OpenDevToolsPanelWindowIntoWindow(7, "network", 7); err == nil {
		t.Fatal("expected open devtools panel window into window to fail")
...
func TestOpenDevToolsPanelWindowIntoWindowRejectsUnknownSourceWindow(t *testing.T) {
	...
	client := Client{baseURL: server.URL, httpClient: *server.Client()}
	if _, err := client.OpenDevToolsPanelWindowIntoWindow(9, "network", 7); err == nil {
		t.Fatal("expected open devtools panel window into window to fail")
...
func TestOpenDevToolsPanelWindowIntoWindowRejectsUnknownTargetWindow(t *testing.T) {
	upgrader := websocket.Upgrader{}
	mux := http.NewServeMux()
	server := httptest.NewServer(mux)
	defer server.Close()
