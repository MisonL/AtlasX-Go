...
func TestOpenDevToolsWindowToWindowsOpensEachSourceTargetInNewWindow(t *testing.T) {
	...
	mux.HandleFunc("/json/list", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`[
			...
			{"id":"dst-1","type":"page","title":"Target","url":"https://chatgpt.com/workspace"},
			{"id":"new-1","type":"page","title":"DevTools One","url":"http://127.0.0.1/devtools/inspector.html?ws=127.0.0.1%3A9222%2Fdevtools%2Fpage%2Fsrc-1"},
			{"id":"new-2","type":"page","title":"DevTools Two","url":"http://127.0.0.1/devtools/inspector.html?ws=127.0.0.1%3A9222%2Fdevtools%2Fpage%2Fsrc-2"}
		]`))
	})
	...
	result, err := client.OpenDevToolsWindowToWindows(9)
	if err != nil {
		t.Fatalf("open devtools window to windows failed: %v", err)
	}
	if result.SourceWindowID != 9 || result.Returned != 2 {
	...
	if len(createURLs) != 2 || !strings.Contains(createURLs[0], "src-1") || !strings.Contains(createURLs[1], "src-2") {
		t.Fatalf("unexpected create target urls: %+v", createURLs)
	}
}

func TestOpenDevToolsWindowToWindowsRejectsUnknownSourceWindow(t *testing.T) {
	upgrader := websocket.Upgrader{}
	mux := http.NewServeMux()
	server := httptest.NewServer(mux)
	defer server.Close()
	...
	client := Client{baseURL: server.URL, httpClient: *server.Client()}
	if _, err := client.OpenDevToolsWindowToWindows(9); err == nil {
		t.Fatal("expected open devtools window to windows to fail")
	} else if !strings.Contains(err.Error(), "window 9 not found") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestOpenDevToolsWindowToWindowsReturnsEmptyWhenSourceWindowHasNoPageTargets(t *testing.T) {
	upgrader := websocket.Upgrader{}
	mux := http.NewServeMux()
	server := httptest.NewServer(mux)
	defer server.Close()
	...
	if result.SourceWindowID != 9 || result.Returned != 0 {
		t.Fatalf("unexpected result: %+v", result)
	}
