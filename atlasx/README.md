...
- 当前侧边栏问答只提供控制面骨架：配置已扩展为 `default provider + provider registry`，并只记录 `api_key_env` 这类环境变量名而不写真实密钥；未配置时 `/v1/sidebar/ask` 显式返回 `503`，即使已配置默认 provider 与 registry 但后端未实现，也会显式返回 `501`。
