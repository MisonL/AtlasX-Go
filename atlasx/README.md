...
- 当前已提供 `tabs agent-execute --confirm <id> <step-id>`，可在 CLI 中基于当前 `agent-plan` 显式确认执行单步动作；当前支持 `sidebar_summarize`、`sidebar_ask` 与 `related_tab`（激活推荐标签），不写 memory、不隐式浏览器跳转，`memory_snippet` 等 preview-only 步骤会显式拒绝。
...
- 当前已提供 `POST /v1/tabs/agent-execute`，可基于当前 `agent-plan` 显式确认执行单步动作；当前支持 `sidebar_summarize`、`sidebar_ask` 与 `related_tab`，`memory_snippet` 等 preview-only 步骤会显式拒绝，且不写 memory。
